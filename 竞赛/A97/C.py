...
class Solution:
    def maximizeWin(self, prizePositions: List[int], k: int) -> int:
        ...
        ans = 0
        counter2 = Counter()
        for i in range(min(counter.keys()), max(counter.keys()) + 1):
            counter2[i] = sum([counter.get(i, 0) for i in range(i, i + k + 1)])
        keys = sorted(counter2.keys(), key=lambda d: counter2[d], reverse=True)
        keys = list(keys)[:100]
        length = len(keys)
        for i in range(length):
            for j in range(i+1, length):
                if keys[j] - keys[i] <= k:
                    diff = sum([counter[i] for i in range(keys[j], keys[i] + k + 1)])
                    s = counter2[keys[j]] + counter2[keys[i]] - diff
                else:
                    s = counter2[keys[j]] + counter2[keys[i]]
                ans = max(s, ans)
        return ans
...
if __name__ == '__main__':
    ...
    k = 220
    solution = Solution()
    res = solution.maximizeWin(prizePositions, k)
    print(res)
